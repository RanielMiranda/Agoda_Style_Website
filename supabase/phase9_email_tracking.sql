-- Phase 9: Email delivery tracking
...
create index if not exists email_delivery_logs_booking_idx
  on public.email_delivery_logs(booking_id);

create index if not exists email_delivery_logs_resort_idx
  on public.email_delivery_logs(resort_id);
