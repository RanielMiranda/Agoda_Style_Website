...
import { getStayCardPrintHtml, getStayCardDownloadHtml } from "./components/ticketPrintUtils";
import {
  TicketHeaderSection,
...
export default function ClientTicketPage() {
  ...
  const stayRows = useMemo(
    () => (stayInfoPayload ? buildStayInfoRows(stayInfoPayload) : []),
    [stayInfoPayload]
  );

  const openPrintEntryPass = () => {
    if (!booking || stayRows.length === 0) return;
    const html = getStayCardPrintHtml(stayRows, booking.id);
  ...
  const downloadTicket = () => {
    if (!booking || stayRows.length === 0) return;
    const html = getStayCardDownloadHtml(stayRows, booking.id);
