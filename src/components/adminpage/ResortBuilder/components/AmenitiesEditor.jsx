...
function SortableAmenity({ id, facility, idx, onRemove, onReplace, onNameChange, safeSrc }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={`flex-shrink-0 w-24 group relative ${isDragging ? "opacity-50" : ""}`}>
      {/* Drag Handle */}
      <div {...attributes} {...listeners} className="absolute -top-2 -left-2 z-10 p-1 bg-white rounded-full shadow-sm border border-slate-100 cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity text-slate-400 hover:text-blue-500">
...
export default function AmenitiesEditor() {
  ...
  const handleBulkUpload = (e) => {
    ...
    const newAmenities = files.map(file => ({ name: "New Amenity", image: file }));
  ...
  return (
    <div id="amenities" className="max-w-6xl mx-auto px-4 mt-8">
      ...
      <input type="file" multiple className="hidden" ref={bulkInputRef} onChange={handleBulkUpload} accept="image/*" />

      <div className="flex gap-4 overflow-x-auto py-4 pb-8 scrollbar-hide">
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={facilities.map((_, i) => `amenity-${i}`)} strategy={horizontalListSortingStrategy}>
